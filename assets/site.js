...
(() => {
  ...
  /* DC edition: power conditioning leads; it is the core program here. */
  const RECORD = [
  ...
  /* the DC page reads power quality first */
  RECORD.unshift(RECORD.pop());
  RECORD[0].t = "Power conditioning examples";
  ...
  /* ---------- the waveform test: distortion, audible ----------
     Synthesized, not sampled: a transformer hums at twice the line frequency.
     Clean power is one pure note; distorted power carries harmonics on top,
  ...
  if (listenWrap && (window.AudioContext || window.webkitAudioContext)) {
    ...
    const start = (mode, btn) => {
      ...
      master.gain.linearRampToValueAtTime(0.4, t0 + 0.4);
      master.connect(ctx.destination);
      /* the hum: 120 Hz fundamental (twice line frequency), plus harmonics
         when the power is dirty */
      const partials = mode === "distorted"
        ? [[120, 0.5], [240, 0.1], [360, 0.3], [600, 0.22], [840, 0.16], [1080, 0.1]]
        : [[120, 0.5], [240, 0.05]];
      const nodes = partials.map(([f, g]) => {
      ...
      if (mode === "distorted") {
        /* the fry: a thin band of noise riding the hum */
        const fry = makeNoise();
        const bp = ctx.createBiquadFilter(); bp.type = "bandpass"; bp.frequency.value = 1400; bp.Q.value = 1.2;
        const fg = ctx.createGain(); fg.gain.value = 0.035;
        fry.connect(bp); bp.connect(fg); fg.connect(master);
        fry.start();
        nodes.push(fry);
      }
      btn.setAttribute("aria-pressed", "true");
      card.classList.add("is-live");
      live = { mode, master, btn, card, bars, raf: 0, nodes };
      const chaos = mode === "distorted" ? 0.55 : 0.12;
      const draw = () => {
        if (!live || live.card !== card) return;
        live.bars.forEach((b, i) => {
          const base = 0.55 + 0.18 * Math.sin(Date.now() / 300 + i * 0.5);
          const j = chaos * Math.sin(Date.now() / (55 + i * 5) + i * 2.1) * Math.random();
          b.style.height = Math.max(8, Math.min(100, (base + j) * 100)) + "%";
        });
